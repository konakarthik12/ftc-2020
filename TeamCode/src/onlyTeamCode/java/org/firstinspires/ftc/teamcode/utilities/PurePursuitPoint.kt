...
class PurePursuitPoint(x: Double, y: Double) : Point(x, y) {
    ...
    companion object {
        ...
        fun getCurvatureOfPoints(leftPoint: PurePursuitPoint, centerPoint: PurePursuitPoint, rightPoint: PurePursuitPoint): Double {
            ...
            val b =
                    0.5 * (leftPoint.x * leftPoint.x - 2.0 * leftPoint.x * k1 + leftPoint.y * leftPoint.y - rightPoint.x * rightPoint.x + 2.0 * rightPoint.x * k1 - rightPoint.y * rightPoint.y) / (rightPoint.x * k2 - rightPoint.y + leftPoint.y - leftPoint.x * k2)
            val a = k1 - k2 * b
